...
# https://stats.stackexchange.com/questions/230171/is-it-meaningful-to-calculate-standard-deviation-of-two-numbers
         s<- data.frame (parametros=sd(x$porc), row.names = "sd")
         t<- data.frame(parametros=c("T.TEST",unlist(t.test(x))))
         testes<-rbind(s,t)
         
         barplot(x$porc, names.arg =c("Especialista","Generalista") , ylim=c(0,100), xlab = "Tipo", col=c("blue","orange"), ylab = "Porcentagem", cex.names = 1.7,cex.axis = 1.7,cex.lab=1.7)
         cat("\n\n\n")  
         print(kable(testes))   
...
###################
  ...
  cat("\n\n\n") 
   ...
   wilcoxtest<- data.frame(unlist(wilcox.test(porc ~ tipo, data=ge) ))
   print(kable(wilcoxtest))


cat("\n\n\n") 
kruskaltest<- data.frame(kruskal=unlist(kruskal.test(porc ~ sistema, data = ge) ))
