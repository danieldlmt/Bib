...
for (it in 1:length(logs$sistema)){
  ...
  name<-(paste("Gen-",as.character(logs$sistema[it]), sep = ""))
...
for (it in 1:length(logs$sistema)){
  ...
  name<-(paste("Esp-",as.character(logs$sistema[it]), sep = ""))
  if(as.character(logs$sistema[it])=="Cocos2d-x"){
    name<-("Gen-Cocos")}
...
ggsave("./graficos/pt/mapa_GenEsp.pdf", p+ theme(axis.title.x=element_blank(), axis.title.y=element_blank()), width=10, height=8)
...
p <- ggplot(MDR, aes(Datastream, Algorithm)) +
  ...
  scale_fill_gradient(low = "white", high = "steelblue")
p + theme(axis.title.x=element_blank(), axis.title.y=element_blank(),text = element_text(size=15))
ggsave("./graficos/en/mapa_GenEsp.pdf", p+ theme(axis.title.x=element_blank(), axis.title.y=element_blank()), width=10, height=8)
